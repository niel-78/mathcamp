...
function parseValue(value) {
    ...
    if (fraction) {
        return new Fraction((fraction[1] ? -1 : 1) * Number(fraction[2]), Number(fraction[3]));
    }
    const slashFraction = /^(-?\d+)\/(\d+)$/.exec(source);
    if (slashFraction) {
        return new Fraction(Number(slashFraction[1]), Number(slashFraction[2]));
    }
...
function getSuggestedValues(values) {
...
export function checkQuestionAnswer(question) {
    ...
    const expectedValues = solveShiftedSquareEquation(question.question) ||
        solveFactorizedEquation(question.question) ||
        solvePolynomialEquation(question.question) ||
        [evaluateArithmetic(question.question)];
    const correctOptions = (question.options || []).filter(option => option.is_correct);
    const parsedCorrect = correctOptions.map(option => parseValue(option.text));

    if (parsedCorrect.some(value => !value) || parsedCorrect.length === 0) {
        const config = typeof question.answer_config === "string"
            ? JSON.parse(question.answer_config || "{}")
            : question.answer_config || {};
        const defaultAnswer = parseValue(config.default_answer);
        if (defaultAnswer && expectedValues.some(expected => expected.equals(defaultAnswer))) {
            return { status: "ok", expected: formatExpectedValues(expectedValues) };
        }
        return {
            status: "mismatch",
            expected: formatExpectedValues(expectedValues),
            suggestionValues: getSuggestedValues(expectedValues),
            suggestion: formatExpectedValues(expectedValues),
            actual: correctOptions.map(option => option.text).join(", ") || "saknas"
        };
    }

    const matches = expectedValues.every(expected => parsedCorrect.some(value => expected.equals(value))) &&
        parsedCorrect.every(value => expectedValues.some(expected => expected.equals(value)));
    return matches
        ? { status: "ok", expected: formatExpectedValues(expectedValues) }
        : {
            status: "mismatch",
            expected: formatExpectedValues(expectedValues),
            suggestionValues: getSuggestedValues(expectedValues),
            suggestion: formatExpectedValues(expectedValues),
            actual: correctOptions.map(option => option.text).join(", ")
        };
}

export function checkBlockAnswerKeys(questions) {
    const result = { checked: [], mismatches: [], unsupported: [] };
    questions.forEach((question, index) => {
        try {
            const check = checkQuestionAnswer(question);
            const item = { question, questionNumber: index + 1, ...check };
            if (check.status === "ok") result.checked.push(item);
            else if (check.status === "mismatch") result.mismatches.push(item);
            else result.unsupported.push(item);
        } catch {
            result.unsupported.push({
                question,
                questionNumber: index + 1,
                status: "unsupported",
                message: "Uttrycket kunde inte tolkas automatiskt."
            });
        }
    });
    return result;
